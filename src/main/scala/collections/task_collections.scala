...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    text.head +: text.tail.collect{
      case n if isASCIIString(n) => n.toUpperCase
      case n if !isASCIIString(n) => n.toLowerCase
    }
  }
  ...
  def numbersToNumericString(text: String): String = {
    ...
    digitsDecoder.foldLeft(text){ case (txt, (in, out)) => txt.replace(in, out) }
  }
